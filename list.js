document.getElementById("sj").addEventListener("click", function() {
  window.location.href = "https://xjvueb.hefaship.com/app/register.php?site_id=1010&topId=4045392";
});
document.getElementById("ff").addEventListener("click", function() {
  window.location.href = "https://bakmry.gziasti.com/app/register.php?site_id=1050&topId=2995191&selfPlanId=1897260";
});
document.getElementById("dmg").addEventListener("click", function() {
  window.location.href = "https://tz.wx-gujdtq3.com/app/register.php?site_id=2228&topId=1351033";
});
document.getElementById("cf").addEventListener("click", function() {
  window.location.href = "https://pkajim.hfjcl.com/app/register.php?site_id=1032&topId=1839709";
});
document.getElementById("t1").addEventListener("click", function() {
  window.location.href = "https://bakmry.gziasti.com/app/register.php?site_id=1051&topId=634000";
});
document.getElementById("28q").addEventListener("click", function() {
  window.location.href = "https://149.30.210.143:35384/#/link?allwin=MIZoqLh9orSEC5L5e%2BKppw%3D%3D";
});
document.getElementById("df").addEventListener("click", function() {
  window.location.href = "https://gfvgx.jgqvji.com/app/register.php?site_id=1030&topId=13806";
});
document.getElementById("ng").addEventListener("click", function() {
  window.location.href = "https://23.235.160.179:7788/#/link?allwin=21931370";
});
document.getElementById("c7").addEventListener("click", function() {
  window.location.href = "https://103.39.18.211:7788/#/link?allwin=7QjUEXXksWwa6da6JQE%2Bsg%3D%3D";
});
document.getElementById("yh").addEventListener("click", function() {
  window.location.href = "https://gfvgx.jgqvji.com/app/register.php?site_id=800&pt=F5EE0AC0-9A90-9737-8193-1EAC89B37114";
});
document.getElementById("zt").addEventListener("click", function() {
  window.location.href = "https://pkajim.hfjcl.com/app/register.php?site_id=1031&topId=34033";
});
document.getElementById("ngty").addEventListener("click", function() {
  window.location.href = "https://nvjmce.hfjcl.com/app/register.php?site_id=1019&topId=11568747";
});
document.getElementById("gjr").addEventListener("click", function() {
  window.location.href = "https://tz.wx-ulslyd12.com/app/register.php?site_id=2304&topId=1206285";
});
document.getElementById("bsj").addEventListener("click", function() {
  window.location.href = "https://wxbsj.agrleo.com/app/register.php?site_id=134523275&topId=398933";
});
document.getElementById("wd").addEventListener("click", function() {
  window.location.href = "https://onkrcy.fnbpsw.com/app/register.php?site_id=1020&topId=2926113";
});
document.getElementById("st").addEventListener("click", function() {
  window.location.href = "https://tfajn.hcjwj.com/app/register.php?site_id=1058&topId=26489&selfPlanId=21600";
});
document.getElementById("xsd").addEventListener("click", function() {
  window.location.href = "https://derear.fnbpsw.com/app/register.php?site_id=1059&topId=57579&selfPlanId=49828";
});
document.getElementById("jl").addEventListener("click", function() {
  alert("君临国际将于本月初上线！");
});

document.getElementById("0").innerHTML="🔥十年信誉平台推荐";

document.getElementById("1").innerHTML="赏金国际";

document.getElementById("2").innerHTML="非凡娱乐";

document.getElementById("3").innerHTML="大满贯";

document.getElementById("4").innerHTML="超凡国际";

document.getElementById("5").innerHTML="Top1体育";

document.getElementById("6").innerHTML="28圈";

document.getElementById("7").innerHTML="巅峰国际";

document.getElementById("8").innerHTML="NG28";

document.getElementById("9").innerHTML="c7";

document.getElementById("10").innerHTML="壹号娱乐";

document.getElementById("11").innerHTML="征途国际";

document.getElementById("58").innerHTML="ng体育";

document.getElementById("59").innerHTML="国际人";

document.getElementById("60").innerHTML="保时捷";

document.getElementById("61").innerHTML="问鼎娱乐";

document.getElementById("62").innerHTML="胜天国际";

document.getElementById("63").innerHTML="新时代";

document.getElementById("64").innerHTML="君临国际";

document.getElementById("57").innerHTML="TC22.MY";


